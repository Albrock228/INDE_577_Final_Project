...
def plot_decision_boundary(X, y, model, title):
    ...
    plt.show(block=True)  # Ensures the plot displays properly
...
def synthetic_test():
    ...
    # AdaBoost Classifier with Decision Tree Weak Learners
    model = AdaBoostClassifier(
        estimator=DecisionTreeClassifier(max_depth=1),  # Replaced base_estimator with estimator
        n_estimators=50,
        random_state=42
    )
    ...
    plot_decision_boundary(X_synthetic, y_synthetic, model, 
                           "Boosting Decision Boundary (Synthetic Data)")
...
def real_data_test():
    ...
    plot_decision_boundary(X, y, model, 
                           "Boosting Decision Boundary (Breast Cancer Data)")
